...
import { MessageCircle, X, Send, Image as ImageIcon, Play, Paperclip } from 'lucide-react';
...
export default function ChatWidget() {
  ...
  return (
    <>
      ...
      <button className={styles.chatBubble} onClick={toggleChat} aria-label="Chat with seller">
        {isOpen ? <X size={24} /> : <MessageCircle size={24} />}
        {!isOpen && unreadCount > 0 && <span className={styles.unreadBadge}>{unreadCount}</span>}
      </button>
          ...
          <div className={styles.chatMessages}>
            {(!chat?.messages || chat.messages.length === 0) && (
              <div className={styles.emptyChat}>
                ...
                <p>Ask any questions about products, orders, or anything else!</p>
              </div>
            )}
